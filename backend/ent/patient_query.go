...
type PatientQuery struct {
	...
	withRightoftreatment *RightoftreatmentQuery
...
// QueryRightoftreatment chains the current query on the rightoftreatment edge.
func (pq *PatientQuery) QueryRightoftreatment() *RightoftreatmentQuery {
	query := &RightoftreatmentQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PatientQuery) QuerySystemmember() *SystemmemberQuery {
...
//  WithRightoftreatment tells the query-builder to eager-loads the nodes that are connected to
// the "rightoftreatment" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PatientQuery) WithRightoftreatment(opts ...func(*RightoftreatmentQuery)) *PatientQuery {
	query := &RightoftreatmentQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withRightoftreatment = query
	return pq
}
...
func (pq *PatientQuery) WithSystemmember(opts ...func(*SystemmemberQuery)) *PatientQuery {
	query := &SystemmemberQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	var (
		...
		loadedTypes = [3]bool{
			pq.withGender != nil,
			pq.withRightoftreatment != nil,
			pq.withSystemmember != nil,
		}
	)
	if pq.withGender != nil || pq.withRightoftreatment != nil || pq.withSystemmember != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withRightoftreatment; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Patient)
		for i := range nodes {
			if fk := nodes[i].Rightoftreatment; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(rightoftreatment.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "Rightoftreatment" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Rightoftreatment = n
			}
		}
	}
	...
	if query := pq.withSystemmember; query != nil {
		...
		nodeids := make(map[int][]*Patient)
		for i := range nodes {
			if fk := nodes[i].Systemmember; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "Systemmember" returned %v`, n.ID)
			}
			for i := range nodes {
