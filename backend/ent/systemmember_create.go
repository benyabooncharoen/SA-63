...
func (sc *SystemmemberCreate) AddPatientIDs(ids ...int) *SystemmemberCreate {
	sc.mutation.AddPatientIDs(ids...)
	return sc
}

// AddPatients adds the patients edges to Patient.
func (sc *SystemmemberCreate) AddPatients(p ...*Patient) *SystemmemberCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (sc *SystemmemberCreate) Save(ctx context.Context) (*Systemmember, error) {
	if _, ok := sc.mutation.SystemmemberName(); !ok {
		return nil, &ValidationError{Name: "systemmemberName", err: errors.New("ent: missing required field \"systemmemberName\"")}
	}
