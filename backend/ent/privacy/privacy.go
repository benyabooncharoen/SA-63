...
// The RightoftreatmentQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type RightoftreatmentQueryRuleFunc func(context.Context, *ent.RightoftreatmentQuery) error

// EvalQuery return f(ctx, q).
func (f RightoftreatmentQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.RightoftreatmentQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.RightoftreatmentQuery", q)
}

// The RightoftreatmentMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type RightoftreatmentMutationRuleFunc func(context.Context, *ent.RightoftreatmentMutation) error

// EvalMutation calls f(ctx, m).
func (f RightoftreatmentMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.RightoftreatmentMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.RightoftreatmentMutation", m)
}
