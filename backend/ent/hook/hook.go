...
// The RightoftreatmentFunc type is an adapter to allow the use of ordinary
// function as Rightoftreatment mutator.
type RightoftreatmentFunc func(context.Context, *ent.RightoftreatmentMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RightoftreatmentFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.RightoftreatmentMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RightoftreatmentMutation", m)
	}
	return f(ctx, mv)
}
